...
## 10. Decisions — D-1 and D-2 taken 19 August 2026, D-3, D-4 and D-5 taken 20 August 2026, D-6 taken 21 August 2026
...
| | Decision | Rationale |
|---|---|---|
| **D-6** | **Delete `spike/`**, taken 21 August 2026. Its two halves were promoted to production: the annotation engine to `errata_ecosystem.goldbuild`, the table-blind predictor to `errata_ecosystem.extractors.TableBlindExtractor` | D-2's fence said the spike would be *deleted or frozen* once gate 2 had a number. It was frozen, for one stated reason: `build_corpus.py` was the only thing that could regenerate `var/spike/corpus.yaml`, and a measured gate whose corpus cannot be rebuilt is a measurement nobody can check. `errata_ecosystem.corpusbuild` now rebuilds it from production code and reproduces the frozen file **byte for byte, field for field, across all 1,426 records** — so the reason expired. See below |
...
### D-6 — the spike is deleted, and what was left in it was never scaffolding

D-2 built a fenced, throwaway grounding spike because gate 2 could not be measured without one and
R1 was gated on gate 2. Rule 5 of that fence said it would be *deleted or frozen* once gate 2 had a
number. It was frozen rather than deleted, and `spike/README.md` gave the reason plainly:
...
That was correct and it left R0's second gate permanently depending on a directory nobody was
allowed to touch or improve. The reason has now expired: `errata_ecosystem.corpusbuild` rebuilds
the gate-2 corpus from production code and reproduces the frozen file **byte for byte, field for
field, across all 1,426 records** — asserted by
`test_the_production_builder_reproduces_the_frozen_gate_2_corpus_exactly`. That exact reproduction,
rather than a matching score, is what made deletion safe: a corpus that merely scored the same
could differ on which records it got right, and 46.34% would then be reproduced by coincidence.

**The more interesting half of this decision is what the spike turned out to contain.** Two of its
three parts were not throwaway at all, and calling them throwaway for another six months would have
been the mistake:

- **The gold builder** is the benchmark's annotation engine. A benchmark needs one permanently, and
  it must *not* be the product's own layout module — `build_gold.py` already said so: "a benchmark
  whose gold can only be regenerated by the code being benchmarked has no independent gold." It is
  now `errata_ecosystem.goldbuild`, and running it reproduces `data/gold/` byte for byte.
- **The table-blind predictor** is the benchmark's baseline. It is the code that produced 46.34%,
  its mechanism is independent of the gold builder's, and a benchmark without a baseline whose
  score is a fixed point cannot tell whether the product improved. It is now
  `errata_ecosystem.extractors.TableBlindExtractor`, frozen with its own parameter table so that
  editing the product's live attribute map cannot silently move a baseline score.
- **`build_corpus.py`** was genuinely throwaway, and it is the part that is gone.

**The version strings keep their old values.** `data/gold/manifest.json` records
`spike-layout/1.0.0` and `spike-gold/1.0.0`, and those strings sit under the annotation files' own
hashes. They read oddly in a package no longer called `spike` and renaming them would silently
invalidate a published manifest to make a module look tidy. A version identifies the algorithm that
produced a hash, not the directory it was sitting in.

`var/spike/` also stays as a path. It is gitignored working data whose location is declared by
`data/reference/manifest.json` and hashed into the R3 reproduction receipt; renaming it would force
every checkout to re-fetch and move a published receipt, in order to correct a directory name
nobody types.
